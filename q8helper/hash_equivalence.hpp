...
: parent(the_load_threshold,thash), root_size(the_load_threshold,thash), histogram(1, thash) {
}


template<class T, int (*thash) (const T& a)>
HashEquivalence<T,thash>::HashEquivalence (const HashEquivalence<T,thash>& to_copy, double the_load_threshold)
: parent(to_copy.parent,the_load_threshold,thash), root_size(to_copy.root_size,the_load_threshold,thash), histogram(1, thash) {
}
...
T HashEquivalence<T,thash>::compress_to_root (T a) {
  ...
  for (const T& x : compress_set) {
    ...
    if (histogram.has_key(compress_set.size())){
        histogram[compress_set.size()]++;
    }
    else {
        //does not have the compress_Set size as a key
        histogram.put(compress_set.size(), 1);
    }

    if (compress_set.size() > max_size){
        max_size = compress_set.size();
    }
...
//  answer << "  max height   : " << max_height() << std::endl;
    answer << "  Maximum size                      :" << max_size << std::endl;
    answer << "  Histogram                         :" << histogram << std::endl;
    answer << "  Total calls to compress_to_root   :" << num_of_calls << std::endl;
    double total;
    double each_entry;
        for (auto number : histogram){
            each_entry = number.first * number.second;
            total += each_entry;
        }

    answer << "  Average size                      :" << total/num_of_calls << std::endl;
  return answer.str();
}
