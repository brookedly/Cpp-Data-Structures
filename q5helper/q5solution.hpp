...
//    std::cout << "AasdasdadaS " << ntn.value << std::endl;
    T greatest = ntn.value;
    for (const NTN<T>& x : ntn.children)
        if (greatest < x.value)
            greatest = maximum(x);
    return greatest;
//  return ntn.value; //Compiles but is incorrect
}
...
std::string longest_word (const DTN& dtn) {
    std::string result;
    if (dtn.is_word == true)  ///checks if the dtn has an actual word
        result = dtn.word_to_here;
    else {
        result = ""; //if not, we will be comparing it with a empty string
    }
    for (auto x : dtn.children) {
        std::string recurs = longest_word(x.second);  //want to determine if the kid is valid
        if (result.size() < recurs.size())  //compares length
            result = recurs;
    }
    return result;
//  return dtn.word_to_here; //Compiles but is incorrect
}
