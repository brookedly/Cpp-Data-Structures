...
bool HashEquivalence<T,thash>::in_same_class (const T& a, const T& b) {
  //Write code here
        if (!parent.has_key(a)){
            ...
            answer << "HashEquivalence::Value a: " << a << " is not in the Equivalence tree";
            throw EquivalenceError(answer.str());
        }
        if (!parent.has_key(b)){
            std::ostringstream answer;
            answer << "HashEquivalence::Value b: " << b << " is not in the Equivalence tree";
            throw EquivalenceError(answer.str());
        }
...
HashSet<HashSet<T,thash>> HashEquivalence<T,thash>::classes () {
  //Write code here
    HashSet<HashSet<T,thash>> answer(1, [](const HashSet<T,thash>& set) { int answer = 1; for (const T& v : set) answer *= thash(v); return answer;});
    HashMap<T,HashSet<T,thash>,(1,thash)> inner_map;

    for (auto letter_pair : parent) {
        T root = compress_to_root(letter_pair.first);
        inner_map[root].insert(letter_pair.first);
    }
    for (auto rs : inner_map)
...
void HashEquivalence<T,thash>::add_singleton (const T& a) {
  //Write code here
        if (parent.has_key(a)){
            std::ostringstream answer;
            answer << "HashEquivalence::add_singleton " << a << " is not a value in the Equivalence tree";
            throw EquivalenceError(answer.str());
        }
        else {
            parent.put(a,a);
...
void HashEquivalence<T,thash>::merge_classes_of (const T& a, const T& b) {
  //Write code here
        if (!parent.has_key(a)){
            std::ostringstream answer;
            answer << "HashEquivalence::merge_classes_of " << a << " is not a value in the Equivalence tree";
            throw EquivalenceError(answer.str());
        }
        if (!parent.has_key(b)){
            std::ostringstream answer;
            answer << "HashEquivalence::merge_classes_of " << b << " is not a value in the Equivalence tree";
            throw EquivalenceError(answer.str());
        }
        ...
        if ( ra < rb)
        {
            ...
            root_size[a_root] = root_size[b_root] + root_size[a_root] ;
...
T HashEquivalence<T,thash>::compress_to_root (T a) {
  //Write code here
    if (!parent.has_key(a)){
        std::ostringstream answer;
        answer << "HashEquivalence::compress_to_root " << a << " is not a value in the Equivalence tree";
        throw EquivalenceError(answer.str());
    }
    ...
    //COMMENT THE SECTION BELOW TO TEST 2D
    for (auto x : values) {
        parent[x] = rt;
    }
    return rt;
