...
T HashGraph<T>::edge_value(NodeName origin, NodeName destination) const {
      if (edge_values.has_key(Edge(origin, destination)))
        ...
        answer << "HashGraph::edge_value Edge pair(" << origin << ", " << destination << ")is not an edge in the graph";
...
int HashGraph<T>::in_degree(NodeName node_name) const {
        if (!node_values.has_key(node_name)){
            std::ostringstream answer;
            answer << "HashGraph::in_degree node_name " << node_name << " is not a node in the graph. In_degree cannot be computed.";
            throw GraphError(answer.str());
        }
        else {
            //return the in-degree of the node_name
            int size = 0;
            for (pair<Edge, T> x: edge_values){
                if (x.first.second == node_name){
                    size++;
                }
            }
            return size;
...
int HashGraph<T>::out_degree(NodeName node_name) const {
        if (!node_values.has_key(node_name)){
            std::ostringstream answer;
            answer << "HashGraph::out_degree node_name " << node_name << " is not a node in the graph. Out_degree cannot be computed.";
            throw GraphError(answer.str());
        }
        else {
            //return the out-degree of the node_name
            int size = 0;
            for (pair<Edge, T> x  : edge_values){
                if (x.first.first == node_name) {
                    size++;
                }
            }
            return size;
...
int HashGraph<T>::degree(NodeName node_name) const {
        if (!node_values.has_key(node_name)){
            std::ostringstream answer;
            answer << "HashGraph::degree node_name " << node_name << " is not a node in the graph. Degree cannot be computed.";
            throw GraphError(answer.str());
        }
        else {
            //return the degree of the node_name
            //need a counter and a for loop
...
auto HashGraph<T>::out_nodes(NodeName node_name) const -> const NodeSet& {
        if (!node_values.has_key(node_name)){
            std::ostringstream answer;
            answer << "HashGraph::out_nodes node_name " << node_name << " is not a node in the graph.";
            throw GraphError(answer.str());
        }
        else {
...
auto HashGraph<T>::in_nodes(NodeName node_name) const -> const NodeSet& {
        if (!node_values.has_key(node_name)){
            std::ostringstream answer;
            answer << "HashGraph::in_nodes node_name " << node_name << " is not a node in the graph.";
            throw GraphError(answer.str());
        }
        else {
...
auto HashGraph<T>::out_edges(NodeName node_name) const -> const EdgeSet& {
        if (!node_values.has_key(node_name)){
            std::ostringstream answer;
            answer << "HashGraph::out_edges node_name " << node_name << " is not a node in the graph.";
            throw GraphError(answer.str());
        }
        else {
...
auto HashGraph<T>::in_edges(NodeName node_name) const -> const EdgeSet& {
        if (!node_values.has_key(node_name)){
            std::ostringstream answer;
            answer << "HashGraph::in_edges node_name " << node_name << " is not a node in the graph.";
            throw GraphError(answer.str());
        }
        else {
...
//        std::cout << "ORIGIN " << origin << " DESTINATION " << destination << " VALUE " << value << std::endl;
//        std::cout << "--------------------------------------- " << std::endl;
//        std::cout << "NodeMap " << node_values << std::endl;
//        std::cout << "--------------------------------------- " << std::endl;
//        std::cout << "EdgeMap " << edge_values << std::endl;
//        std::cout << "--------------------------------------- " << std::endl;
...
//        node_values.erase(node_name);
        //Both above do the same thing but does not remove the node D entirely. LocalInfo sets are left empty.
        //WAIT JK IT DOES. Look at how the copy node map prints
...
//        std::cout << "EdgeMap " << edge_values << std::endl;
//        std::cout << "--------------------------------------- " << std::endl;
        for (pair<NodeName, LocalInfo> x : node_values)
        {
            LocalInfo copy_local_info = x.second; //copy of the LocalInfo
            for (auto value : copy_local_info.out_nodes)
            {
                if (value == node_name){
                    node_values[x.first].out_nodes.erase(value);
                }
            }

            for (auto value : copy_local_info.in_nodes){
                if (value == node_name){
                    node_values[x.first].in_nodes.erase(value);
                }
            }

            for (auto value : copy_local_info.out_edges){
                if (value.second == node_name){
//                    node_values[x.first].out_edges.erase(value);
                    remove_edge(value.first, value.second);
                }
            }

            for (auto value : copy_local_info.in_edges){
                if (value.first == node_name) {
//                    node_values[x.first].in_edges.erase(value);
                    remove_edge(value.first, value.second);
                }
            }
        }
...
//        std::cout << "EdgeMap " << edge_values << std::endl;
//        std::cout << "--------------------------------------- " << std::endl;
//        std::cout << "NODEMAPCOPY " << nodemapcopy << std::endl;
//        std::cout << "--------------------------------------- " << std::endl;
//        std::cout << "FINAL NODE MAP " << node_values << std::endl;
    }
...
void HashGraph<T>::remove_edge (NodeName origin, NodeName destination) {
        ...
        if (edge_values.has_key(Edge(origin, destination)))
        {
            ...
            //edge_values does not have the Edge pair
            return;
        }
    ...
    if (!in_file.is_open())
    {
        std::cout<< "Sorry, but this file does not exist" << std::endl;
            ...
            while (getline(in_file, line)) {
                std::vector<std::string> whole_line = ics::split(line, separator);
                if (whole_line.size() == 1) {
                    add_node(line);
                } else {
                    for (int x = 0; x < whole_line.size(); x++) {
                        std::istringstream(whole_line.at(2)) >> value;
                        add_edge(whole_line.at(0), whole_line.at(1), value);
                    }
                }
            }
        }
...
void HashGraph<T>::store(std::ofstream& out_file, std::string separator) {
        if (!out_file.is_open()) {
            ...
            for (pair<NodeName, LocalInfo>  pair : node_values) {
                out_file << pair.first << std::endl;
            }
            for (pair<Edge, T>  pair : edge_values) {
                out_file << pair.first.first << separator << pair.first.second << separator << pair.second << std::endl;
            }
        }
}
...
//        std::cout << "Right hand side " << rhs.node_values << std::endl;
        for (auto pair : rhs.node_values){
            for (auto p : node_values){
                p.first = pair.first;
...
//        std::cout << "Right hand side " << rhs.node_values << std::endl;
//        std::cout << "-------------------" << std::endl;
//        std::cout << "Node values " << node_values << std::endl;
        if (this == &rhs)
            return true;
        return node_values == rhs.node_values && edge_values == rhs.edge_values;
//        return node_values == rhs.node_values;
}
...
std::ostream& operator<<(std::ostream& outs, const HashGraph<T>& g) {
    ics::HeapPriorityQueue<typename HashGraph<T>::NodeLocalEntry, HashGraph<T>::LocalInfo_gt> result;
    ...
    for (auto pair : result){
        outs << "  " << pair.first << " -> " << pair.second << std::endl;
    }
    outs << "]";
    return outs;
}
